    ...
    <div className="space-y-10">
      ...
      {!result ? (
        ...
        >
          <div 
            {...getRootProps()} 
            className={`glass-card p-16 border-2 border-dashed text-center cursor-pointer transition-all duration-500 group relative overflow-hidden ${
              isDragActive 
          ...
          <div className="glass-card p-8 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {result.suggestions.map((s, i) => (
                <div 
                  key={i} 
                  className={`relative p-6 rounded-3xl border-l-4 transition-all duration-300 group hover:translate-x-1 ${
                    s.category === 'Critical' ? 'border-l-rose-500 bg-rose-500/5' : 
                    s.category === 'Important' ? 'border-l-amber-500 bg-amber-500/5' : 
                    'border-l-indigo-500 bg-indigo-500/5'
                  }`}
                >
                  <div className="flex items-center gap-3 mb-3">
                    <span className={`text-[10px] font-black px-2 py-0.5 rounded-md uppercase tracking-[0.2em] ${
                      s.category === 'Critical' ? 'bg-rose-500/20 text-rose-400' : 
